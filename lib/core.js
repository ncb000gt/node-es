...
module.exports = function (config, req, self) {
	...
	self.bulk = function (options, commands, callback) {
		...
		var
			...
			path = (options._index || '') +
				...
				(options._index ? pathAppend('_bulk') : '_bulk'),
			serializedCommands = '';
	...
	self.count = function (options, query, callback) {
		...
		var
			...
			path = (options._index || config._index || '') +
				(options._index || config._index ? pathAppend(options._type || config._type) : '') +
				(options._index || config._index ? pathAppend('_count') : '_count');
	...
	self.registerPercolator = function (options, query, callback) {
		...
		var
			...
			path = '_percolator' +
				pathAppend(options._index || config._index) +
